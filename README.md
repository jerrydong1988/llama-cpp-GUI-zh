...
>
> ⚠️ **v1.5.0 重要变更**：仅支持最新版 llama.cpp（≥ commit b39a7bf）。`--spec-type` 旧值 `mtp` 已替换为 `draft-mtp`，内置 Web 界面已移除。旧引擎用户请使用 v1.4.0 及之前版本。
>
...
- **状态监控**：运行中 / 连接中断自动标识
- **实时日志**：服务器输出实时监控
- **API 访问**：一键打开 API 地址（新版 llama.cpp 已无内置 Web 界面，推荐使用外部客户端）

### 🎛 配置管理
- **命名配置**：保存多个命名配置（如 `draft_qwen`、`draft_gemma`），下拉框快速切换
